...
April 2026 (Draft v0.63)

---

## Abstract

For the stochastic damped harmonic oscillator (SDHO) governing liquid financial markets [1, 2], we derive a closed-form identity relating the deterministic variance fraction $R^2$ to the dissipation rate $\Omega$ and mean-reversion coefficient $k$. The identity follows from the discrete Lyapunov equation at stationarity rather than from a fit, with the leading-order form $R^2 = \Omega^2/(1+\Omega^2)$ emerging as its first-order Taylor expansion around the critical point $(\Omega = 1, k = 0)$. The reported $R^2$ is the variance share of $\Delta y$ carried by the SDHO drift, not a measure of return predictability or tradable alpha. Three geometric structures place the identity in proper context, namely, the contact-extended entropy production $\dot{s} = \Omega y^2$ of stochastic thermodynamics, which identifies $\Omega$ as the dissipation coupling constant; the Fisher information metric on $(k, \Omega, \sigma)$, unique by Čencov's theorem and diagonal; and the fluctuation-dissipation theorem (FDT) at stationarity. Twenty-three liquid markets spanning five asset classes and four continents sit on the identity within $4.5 \times 10^{-4}$ in $R^2$, and a candidate sweep identifies Pakistan's KSE-100 at $\Omega = 0.978 \pm 0.027$, straddling the critical point. Short-horizon anti-persistence, the variance risk premium, and the missing quantitative content of the adaptive markets hypothesis read as structural corollaries, providing a quantitative perspective on the random-walk-versus-structure debate at an interior operating point $R^2 \approx 0.57$.
...
Three features of the OU-at-stationarity setup each admit a geometric formulation, making the step from dynamical system to geometry shorter than it first appears. *What geometry governs a dissipative flow?* Not the symplectic geometry of Hamiltonian mechanics, but its generalization to systems with an entropy coordinate, contact geometry [20], in which $\Omega$ appears as the coupling constant of entropy production $\dot{s} = \Omega y^2$ of stochastic thermodynamics, identifying the object around which the derivation is organized. *What is the natural metric on the parameter family?* By Čencov's theorem [13], with prior work by Rao [18] and the systematic modern treatment in Amari [19], the Fisher information metric is the uniquely invariant Riemannian structure on $(k, \Omega, \sigma)$. *What balance closes a driven-dissipative system at stationarity?* The fluctuation-dissipation theorem (FDT) [14, 15], which couples diffusion to the phase-space contraction from dissipation.

The result is an algebraic identity between $R^2$ and the SDHO parameters that follows from the discrete Lyapunov equation at stationarity, with no fitted inputs. The full closed form is $R^2_{\rm exact}(k, \Omega) = (\Omega^2 - \tfrac{3}{2} k \Omega + \tfrac{1}{2} k^2 + k)/(2\Omega - k)$, derived in Appendix A; at the critical point $(\Omega = 1, k = 0)$ its first-order Taylor expansion is

$$R^2 \;=\; \frac{\Omega^2}{1 + \Omega^2} \tag{1}$$

with the critical point itself at $R^2 = 1/2$ separating deterministic-dominated from stochastic-dominated regimes. The succinct form (1) is the leading-order expression in the small-$k/\Omega$ regime that holds across all tested markets, and it is the form derived from contact-geometry in §3.1; the exact form of Appendix A is what the cross-market data matches to $10^{-4}$. The reported $R^2$ is the deterministic fraction of momentum-acceleration variance, the share of $\Delta y$ predictable from current phase-space state under the SDHO drift, and is not a measure of return predictability or tradable alpha. This paper is the third in a series [1, 2] that derived the SDHO structure by applying phase-space methods and statistical mechanics to market dynamics, and the geometric results reported here are the culmination of that earlier groundwork.
...
But the theorem has more to deliver than its own validation. We note especially that three long-standing market phenomena have been treated in separate literatures but each leaves open structural questions. For example, short-horizon anti-persistence, documented in Hurst-exponent measurements below $H = 0.5$ for decades [10, 11], has been attributed to microstructure and bid-ask bounce but lacks a first-principles derivation; here it appears as the linearized geodesic restoring force on the Gaussian statistical manifold (§2.2), with magnitude fixed by the Fisher metric. The variance risk premium [12], the persistently large gap between implied and realized volatility, admits rational-compensation accounts that get the existence of the premium but not its magnitude; here the magnitude is constrained directly by the cross-market $R^2$ (§3.5), as the cost of pricing the SDHO-predictable component as if it were pure noise. The adaptive markets hypothesis (AMH) [8, 9] reframed market efficiency as dynamic equilibrium but supplied no numerical target for the equilibrium value; here the two-population Nash equilibrium between arbitrageurs and liquidity providers places the operating point on the geometric frontier at $R^2 \approx 0.57$ (§4.2). Each of the three literatures has a different gap; the same entropy-constrained geometric source supplies a structural constraint on all three, and the efficient-markets debate [4, 5, 6, 7, 8, 9] is correspondingly constrained: neither $R^2 = 0$ nor $R^2 \to 1$ but an interior operating point ($R^2 \approx 0.57$) where the market carries substantial SDHO structure and simultaneously retains genuine noise.

Each of the three structures is mature and has its own literature, but their simultaneous imposition on a Gaussian stationary distribution, and the consequence that $R^2 = \Omega^2/(1+\Omega^2)$ is an identity rather than a fit, is what is new here. The finance-side literatures play a different role. Microstructure theory [22, 23] and earlier econophysics [6, 10, 11] supply the objects on which this geometric machinery acts and the prior phenomenology against which it is measured; neither is a source of the theorem. Further geometric implications of the same Fisher and contact structure, the volatility smile as Nijenhuis curvature of the Kähler triple of §2.2 and the critical-VIX transition as a Fokker-Planck spectral-gap closure, are developed in companion papers [16, 17].

The information-geometric and stochastic-thermodynamic treatments of markets that do exist aim at different targets. Taylor [29] uses the Fisher metric to define a geodesic distance between generalized-Pareto and generalized-extreme-value fits of equity return distributions, enabling a risk-focused clustering of securities; the Fisher metric plays a similarity-measure role on return-distribution space rather than as a structural-constraint on a dynamical system. Touzo, Marsili, and Zagier [30] derive a Szilárd-engine analogy for the Glosten-Milgrom microstructure model, giving a generalized second-law inequality internal to a specific microstructure model rather than a cross-market empirical identity. Georgiev [31, 32] applies the Seifert stochastic-thermodynamics framework directly to price time series, decomposing total entropy change into medium and market components with empirical verification of the second-law inequality. And Jha [33] imports stochastic thermodynamics into price-impact and round-trip-arbitrage analysis, proving a financial-second-law bound on expected round-trip profit.

None of these four produces a closed-form, parameter-free algebraic identity between a market's dissipation rate and its deterministic variance fraction, nor a cross-market empirical test of such an identity to $10^{-4}$ precision. The present paper occupies that specific gap: the joint action of Čencov uniqueness, contact-geometric entropy production, and the FDT on the SDHO places equation (1) in canonical geometric form, and the identity is validated across twenty-three markets spanning five asset classes.

### 1.1 Organization of the Paper

The remainder of the paper is organized as follows. Section 2 reviews the Fisher information metric on the SDHO parameter space and identifies the Gaussian statistical manifold underlying the dynamics. Section 3 derives the Pareto frontier $R^2 = \Omega^2/(1+\Omega^2)$ from contact geometry, validates it across the six futures markets of [1], grounds the operational market meaning of $\Omega$ (§3.3), traces the cross-market $R^2$ stability to Fisher diagonality (§3.4), and reads the variance risk premium as a structural corollary (§3.5). Section 4 develops the economic interpretation as Pareto-optimal and supplies a two-population selection argument for the operating point. Section 5 collects the framework's predictions and their empirical status. Section 6 discusses limitations (§6.1), the principal falsifiable prediction in the underdamped regime $\Omega < 1$ (§6.2), and what the framework opens up beyond the present paper (§6.3). Section 7 concludes. Appendix A gives a complementary discrete-time Lyapunov derivation that produces the exact closed form $R^2_{\rm exact}(k, \Omega)$. Appendix B reports the seventeen-market cross-market validation. Appendix C establishes the Pareto-frontier rigor by characterizing the achievable set and proving non-dominance.
...
at the FDT normalization $\Omega = 1$, where the discrete-time ratio $\sigma^2 / \operatorname{Var}(y) = \Omega(2-\Omega)$ equals unity. Equation (3) is therefore the critical-point specialization of the discrete-time FDT, displayed here to expose the geometric structure in the simplest form; the full discrete-time fluctuation-dissipation relation without any critical-point specialization is used in Appendix A's derivation, and equation (1) below emerges as the first-order Taylor expansion of that derivation's output, not as a standalone consequence of the $\Omega = 1$ normalization. The $\sigma^2 = 2\Omega\,\operatorname{Var}(y)$ continuous-time version of the same balance is sourced rigorously by the stationary Fokker-Planck calculation developed in [2], §3.1 as a finite-volume discretization on the empirical Voronoi-Delaunay tessellation of phase space, to which we refer for the full machinery.
...
recovering the Pareto frontier (1).
...
Why call this geometric rather than merely algebraic? Because none of the inputs are modeling choices. $\Omega$ enters not as a fitted parameter but as the trace of the SDHO drift matrix and the coupling constant of the contact entropy production as given by (2). The variance equality (3) is a consequence of stationarity on the contact manifold at $\Omega = 1$, not an assumption imposed from outside. And the partition (4) inherits its form from the Fisher-metric diagonality of §2.1, which isolates $\Omega$ from $k$ and $\sigma$ by Čencov's theorem.

Two leading-order simplifications underlie the chain and should be named: dropping the $kx$ term in the DHO-variance numerator (the small-$k/\Omega$ step (4a)), and using the FDT at its critical-point normalization $\Omega = 1$ (equation (3)). Both are valid to leading order but not to higher orders, and a rigorous comparison with data requires the discrete-time Lyapunov closed form without either approximation. Appendix A derives the exact form as

$$R^2_{\rm exact}(k, \Omega) \;=\; \frac{\Omega^2 - \tfrac{3}{2}k\Omega + \tfrac{1}{2}k^2 + k}{2\Omega - k}$$

(equation (A.1)), which agrees with (1) at $(\Omega = 1, k = 0)$ in both value and first partial derivatives: (1) is therefore the first-order Taylor expansion of $R^2_{\rm exact}$ around that point. The two forms carry different content: (1) displays the Fisher-diagonality of §2.1 as a single-parameter reduction and makes the critical point $(\Omega = 1, R^2 = 1/2)$ explicit, while (A.1) supplies the empirical precision that the overlapping-window regression requires. Figure 1 plots (1); Figure 2 plots (A.1), where the cross-market residuals tighten from $\sim 10^{-2}$ to $\sim 10^{-4}$ (§3.2).
...
The theorem makes a single quantitative prediction: for any system whose parameters are fit to the SDHO, the measured deterministic fraction should lie on the curve $R^2 = \Omega^2/(1+\Omega^2)$. What this actually tests is worth stating explicitly: the identity is tautological for any stationary linear SDHO with white noise, so the empirical result is not that markets satisfy the identity by construction, but that real markets *are* stationary linear SDHOs with white noise to the measured precision. The cleanest available test bed is liquid financial markets, where [1] established by sparse identification (SINDy) that momentum dynamics follow the SDHO and [2] extended the empirical foundation across temporal scales and 462 years of equity history. The test is non-trivial because the SDHO parameters $(k, \Omega)$ and the statistic $R^2$ are estimated independently: $\Omega$ appears as a coefficient in the sparse SINDy regression, while $R^2$ is the coefficient of determination of that regression, and the functional relation between them is not imposed during fitting. Table 1 collects the validation results for the six futures markets of [1], Table 2, using the values as originally reported.
...
All six markets sit on the geometric frontier within $|R^2_{\rm data} - R^2_{\rm frontier}| \leq 0.011$, with no market-specific tuning. An independent test extends this validation to seventeen additional markets drawn from free daily data, covering Asian, European, and emerging-market equity indices along with four cryptocurrencies, and every market sits on the same frontier within the same tolerance; the full market list, residuals, and reproducibility details are in Appendix B. The point-estimate residuals against the contact-geometric form (1) are at most $7 \times 10^{-3}$, and against the rigorous discrete-time form (A.1) of Appendix A they tighten by four orders of magnitude to at most $4.3 \times 10^{-4}$. Within the bootstrap uncertainty of the SDHO fits ($\pm 1$ standard deviation on $R^2$ of order $\pm 0.008$ from $n_\text{boot} = 1000$ resamples per market), every market is statistically consistent with sitting exactly on the geometric frontier. Across all twenty-three markets, $\Omega \in [1.058, 1.210]$ with $\text{CV}(\Omega) = 3.8\%$ and $R^2 \in [0.535, 0.590]$ with $\text{CV}(R^2) = 2.9\%$. The companion paper [2] supplies the complementary robustness axis: the same identity holds across 5-day to 252-day aggregation windows for SPY with residuals below 0.3%, across VIX quintiles at the same precision, and across non-overlapping as well as overlapping regression constructions; the present panel adds cross-market breadth to [2]'s temporal and regime-conditional robustness.

**Decomposition of market dynamics at the operating point.** The cross-market cluster at $R^2 \approx 0.57$ has a direct reading in terms of the SDHO Langevin equation of §1. Taking the six-futures mean values $(k, \Omega) = (0.22, 1.16)$ from Table 1 as representative empirical parameters, the one-step increment of momentum decomposes as

$$dy \;=\; \underbrace{-0.22\, x \, dt - 1.16\, y \, dt}_{\text{DHO (deterministic), } R^2 \approx 57\%} \;+\; \underbrace{\sigma \, dW}_{\text{noise, } \approx 43\%}, \tag{5}$$

where the coefficients are the empirical cross-market means of $(k, \Omega)$ and the percentages are the variance shares. These coefficients are measured quantities reported in [1] and Table 1 rather than parameters of the theory: the theoretical content of §3.1 is the functional identity $R^2 = \Omega^2/(1 + \Omega^2)$, which is parameter-free, and the observation that real markets cluster near $\Omega \approx 1.16$ is a separate empirical fact that receives its own mechanistic reading in §4.2. Approximately 57% of the variance of $\Delta y$ is therefore carried by the DHO drift and 43% by the stochastic forcing; the market exhibits substantial DHO structure and simultaneously retains genuine stochasticity. This mixture is representative across the full twenty-three-market panel.
...
Before tracing the stability of $R^2$ through Fisher diagonality (§3.4), it is worth stating what $\Omega$ *is* in market terms. Operationally, $\Omega$ is the rate at which a momentum perturbation $y = \dot{x}$ decays back toward equilibrium under the SDHO drift alone, measured in units of the SINDy time step: a shock $\delta y$ contracts to $\delta y \, e^{-\Omega \Delta t}$ one step later. The cross-market value $\Omega \approx 1.16$ corresponds to a momentum half-life of approximately $\ln 2 / \Omega \approx 0.6$ time steps, with the step set by the aggregation level chosen for the underlying regression. This operational reading sits alongside the geometric ones introduced in §3.1 (trace of the negative drift matrix, coupling constant of entropy production, magnitude of the Liouville violation); the four are identical up to multiplicative constants.

The empirical timescales this produces are consistent with independent measurements of market mean-reversion. At hourly aggregation, the SDHO eigenvalues computed in [1], §6.1 give momentum decay timescales of 1 to 4 hours, matching the mean-reversion half-lives reported by Safari and Schmidhuber [24]. At daily aggregation the same regression produces $\Omega \approx 1.16$ with decay timescales in daily units, covering the 5-day to 252-day aggregation ladder of [2]. What controls $\Omega$ microstructurally is a distinct question: arbitrage pressure against momentum shocks, liquidity-provider inventory mean-reversion, and behavioral herding are all candidate mechanisms, and the two-population account of §4.2 invokes the first two. The cryptocurrency markets of Appendix B, however, sit on the frontier as tightly as the institutional futures despite lacking a designated market-maker population, which constrains any mechanism producing $\Omega \approx 1.16$ to operate without requiring an arbitrageur/liquidity-provider institutional split. Which specific channel produces the observed $\Omega$ across such structurally dissimilar instruments is an open question, returned to as a limitation in §6.1.
...
Empirically, this isolation manifests as $\text{CV}(\Omega) = 3.8\%$ across the twenty-three markets, where the scaled restoring force $\Phi = k\,\text{Var}(x)$ varies by more than an order of magnitude across the futures panel (from 0.016 for 6E to 0.292 for CL) yet $\Omega$ does not. Near-universality of $\Omega$ then implies near-universality of $R^2$ via equation (1). Every link in the chain is geometric, not phenomenological:

$$\text{Čencov} \;\Rightarrow\; \text{diagonal Fisher metric} \;\Rightarrow\; \Omega \text{ Fisher-orthogonal} \;\Rightarrow\; \Omega \text{ near-universal} \;\Rightarrow\; R^2 \text{ near-universal}.$$

A clarification on the scope of this chain: geometric isolation on the parameter manifold implies that the conditional MLE for $\Omega$ is robust within a single market: a perturbation in $k$ or $\sigma$ does not propagate into the estimate of $\Omega$ at leading order. The cross-market clustering of $\Omega$ at $\text{CV} \approx 4\%$, however, additionally requires a selection argument that explains why different markets, each with their own $k$ and $\sigma$, end up at nearly the same $\Omega$. We supply that argument in §4.2 through the two-population Nash equilibrium between arbitrageurs and liquidity providers. The Fisher diagonality of §2.1 is what ties $R^2$-clustering to $\Omega$-clustering once the latter is in hand; the selection argument of §4.2 is what produces the $\Omega$-clustering in the first place.

The frontier curve also has a natural geometric interpretation. At $\Omega = 1$, the formula gives $R^2 = 0.5$, meaning that the dynamics are equally split between DHO structure and noise. For $\Omega > 1$ the DHO dominates, and for $\Omega < 1$ the noise dynamics dominate, so $\Omega = 1$ is the critical point separating the two regimes. The empirical observation that all twenty-three markets in the primary panel sit at $\Omega \in [1.06, 1.21]$ places the entire set slightly above this critical point. The candidate sweep of §6.2 extends this picture by identifying Pakistan's KSE-100 at $\Omega = 0.978 \pm 0.027$, straddling the critical point and sitting on the exact frontier to within $10^{-3}$ in $R^2$; whether any liquid market exists measurably below $\Omega = 1$ remains open.
...
Beyond the structural VRP corollary of §3.5, the frontier curve $R^2 = \Omega^2/(1+\Omega^2)$ admits a further economic insight that illuminates why the cross-market cluster sits where it does. In portfolio theory, the Markowitz efficient frontier in $(\sigma, \mu)$ space represents the maximum return achievable for a given level of risk, its shape determined by the correlation structure of asset returns, and investors select a point on it through preference, with the tangent portfolio Sharpe-maximizing. In the SDHO framework, $R^2 = \Omega^2/(1+\Omega^2)$ plays a structurally identical role: it represents the maximum deterministic fraction achievable for a given dissipation rate, its shape is determined by the Fisher metric, and the market selects an operating point through economic equilibrium. Both frontiers are boundaries of achievable sets, and both have equilibrium points whose location reflects competitive selection. We therefore adopt the Pareto-frontier terminology for the $R^2(\Omega)$ curve, with $R^2 \approx 0.57$ at $\Omega \approx 1.16$ as its empirical operating point. Appendix C establishes the terminology as rigorous rather than analogical by setting up the predictability-versus-dissipation trade-off as a formal multi-objective optimization, characterizing the achievable set of stationary SDHO realizations, and proving non-dominance via $\partial R^2_{\rm exact}/\partial \Omega > 0$ throughout the empirical region; the Markowitz analogy is exact in the multi-objective optimization sense.
...
The chain of §§2–4 produces the Pareto frontier and its corollaries from a single geometric source, and the corollaries are not proved here so much as offered as consistent interpretations of the same Fisher-metric structure. The three subsections below separate what the theorem stands on from what remains provisional, and where a clean falsification test can be implemented.
...
The Pareto frontier is one result from machinery built to be general. The Fisher-metric and contact-geometric structures assembled in §§2-3 carry more information than the single-parameter reduction that produced equation (1), and a handful of related phenomena are natural next targets, each under development in separate papers. For example, the volatility smile admits a curvature-based reading as a departure from the flat-space Black-Scholes-Merton limit of the Kähler triple introduced in §2.2. The critical-VIX threshold of [2] admits a reinterpretation as a Fokker-Planck spectral-gap closure, with volatility clustering following as critical slowing down in the sense of Hohenberg and Halperin's classification of dynamic critical phenomena [27]. The position of $(k, \Omega, R^2)$ in the cross-market cluster can also serve as a volatility-regime indicator distinguishing calm, transitional, and crisis periods [28]. And the $k(\tau)$ power-law decay documented in [2] invites a first-principles account from information accumulation on the statistical manifold. Each of these builds on the Pareto-frontier result established here rather than replacing it.

---

## 7. Conclusion

A geometric theory earns its keep by forcing structure that would otherwise have to be fitted. The discrete Lyapunov equation, applied to any stationary linear SDHO with white noise, produces the algebraic identity $R^2_{\rm exact}(k, \Omega) = (\Omega^2 - \tfrac{3}{2}k\Omega + \tfrac{1}{2}k^2 + k)/(2\Omega - k)$, with the leading-order form $R^2 = \Omega^2/(1+\Omega^2)$ as its first-order Taylor expansion around the critical point. Three geometric structures put this identity in canonical form, none of them chosen: the contact-extended entropy production $\dot{s} = \Omega y^2$, which makes $\Omega$ the coupling constant of stochastic-thermodynamic dissipation; the Fisher metric, unique on a statistical manifold by Čencov's theorem and diagonal in $(k, \Omega, \sigma)$ for the SDHO; and the fluctuation-dissipation theorem, which couples the stochastic variance to the state variance at stationarity. The critical point $R^2 = \tfrac{1}{2}$ at $\Omega = 1$ is the boundary between deterministic-dominated and stochastic-dominated regimes.

For empirical validation we find that twenty-three liquid markets spanning five asset classes and four continents sit on the exact identity within $4.5 \times 10^{-4}$ in $R^2$, with cluster center $(\Omega, R^2) \approx (1.12, 0.57)$ and coefficients of variation $\mathrm{CV}(\Omega) = 3.8\%$ and $\mathrm{CV}(R^2) = 2.9\%$ across the full panel. The four cryptocurrency markets sit on the curve as tightly as the institutional futures, despite lacking the market-maker microstructure originally invoked to explain the clustering, which indicates that the geometry is what places markets on the frontier and the microstructure is at most selecting the operating point. The framework thereby supplies a quantitative constraint on the random-walk-versus-structure debate: markets neither follow a random walk ($R^2 = 0$) nor exhibit exploitable structure in the strong sense ($R^2 \to 1$) but sit at an interior geometric fixed point. The variance risk premium, short-horizon anti-persistence, and the missing quantitative content of adaptive markets each read as structural corollaries of the same entropy-constrained source rather than as independent puzzles.

The most useful thing a theorem like this does is expose where it could go wrong. The twenty-three markets of the primary panel all sit in the overdamped region $\Omega > 1$, and the frontier formula makes quantitative predictions across the full domain $\Omega \in (0, \infty)$ that the panel alone cannot probe. The candidate sweep of §6.2 takes a first step into that territory: Pakistan's KSE-100 fits at $\Omega = 0.978 \pm 0.027$, straddling the critical point, and sits on the exact frontier within the bootstrap uncertainty, extending the tested domain to the boundary of the underdamped regime and supporting the universality of the geometric constraint across the critical point. A liquid market with $\Omega$ measurably below unity, rather than straddling it, remains the cleanest strict-underdamped test: if it sits on the curve, the geometric constraint is universal across dissipative market dynamics; if it sits off, the derivation of §3.1 is wrong and we will want to know how. Finding such a market, together with the companion work on manifold curvature as a source of the volatility smile, and critical phenomena as a source of volatility clustering, is what this result opens up.
...
The empirical $(\Omega_i, k_i, R^2_i)$ inputs for the Table 1 futures markets are taken directly from [1]; the daily price data for Tables B1, B2, and B3 (Appendix B) and the §6.2 underdamped candidate sweep are freely available from Yahoo Finance and are downloaded at runtime via the `yfinance` Python package. All code, figures, and working draft are available at https://github.com/bruce3141/Information-Geometry-Market-Dynamics (archived at Zenodo, https://doi.org/10.5281/zenodo.19681936). The repository reproduces, end-to-end in about three to four minutes on a standard laptop, Tables B1 and B2 (cross-market validation), Table B3 (four-way Lyapunov-consistency battery of §B.5), the phase-plane flow alignment of Figure 3 (§B.6), the §6.2 underdamped candidate sweep including the KSE-100 critical-point result, the four numerical Pareto-frontier verifications of Appendix C, the §4.2-footnote Sharpe-optimization demonstration, and all paper figures. The pipeline includes a built-in sanity check against [2], Table 7a (SPY 1993–2025 at $n=5$ daily, overlapping, linear) reproducing the published $(k, \Omega, R^2)$ to four decimal places. All analysis was performed in Python using NumPy, Pandas, SciPy, Matplotlib, and yfinance, with versions pinned in the repository's `pyproject.toml`.

## Acknowledgments

I thank the institutional-investor contact who raised applications of the SDHO phase-space framework to regime-conditional portfolio construction, whose questions sharpened the two-population reading of §4.2; and the independent researcher whose out-of-sample replication of the Pareto identity at a regression setting beyond those reported here influenced the confidence with which the §6.2 underdamped prediction is stated. I am grateful to the quantitative-finance community on X for ongoing discussions of the SDHO phase-space framework and its cross-market tests; several threads there surfaced the candidate markets ultimately tested in §6.2. The broader information-geometry, contact-geometry, and stochastic-thermodynamics communities whose work this paper builds on are cited throughout. Any errors are my own.
...
33. Jha, A.K. A Stochastic Thermodynamics Approach to Price Impact and Round-Trip Arbitrage: Theory and Empirical Implications. *arXiv* **2025**, arXiv:2512.03123.
...
The seventeen new markets extend the $\Omega$ range from $[1.12, 1.21]$ in the original six futures to $[1.058, 1.210]$ across the full twenty-three-market panel, with the seventeen new markets themselves spanning $\Omega \in [1.058, 1.177]$. Every market in this panel remains in the overdamped region $\Omega > 1$, so the regime $\Omega \leq 1$ is unprobed by the panel; the candidate sweep of §6.2 addresses that gap, identifying Pakistan's KSE-100 at $\Omega = 0.978 \pm 0.027$ on the exact frontier.
