...
Three features of the OU-at-stationarity setup each admit a canonical geometric formulation, making the step from dynamical system to geometry shorter than it first appears. *What geometry governs a dissipative flow?* Not the symplectic geometry of Hamiltonian mechanics, but its generalization to systems with an entropy coordinate: contact geometry [20], in which $\Omega$ appears as the coupling constant of entropy production $\dot{s} = \Omega y^2$ of stochastic thermodynamics, identifying the object around which the derivation is organized. *What is the natural metric on the parameter family?* By Čencov's theorem [13], with foundational prior work by Rao [18] and the systematic modern treatment in Amari [19], the Fisher information metric is the unique invariant Riemannian structure on $(k, \Omega, \sigma)$. *What balance closes a driven-dissipative system at stationarity?* The fluctuation-dissipation theorem (FDT) [14, 15], which couples the diffusion to the phase-space contraction from dissipation. The theorem of §3 is what these three structures together place in canonical form.
...
The seventeen new markets broaden the $\Omega$ range from $[1.12, 1.21]$ in the original six futures to $[1.058, 1.177]$ across the full set, yet every market remains in the overdamped region $\Omega > 1$. The broader range widens the span of dissipation rates over which the frontier identity is tested; the confinement to overdamped dynamics leaves the regime $\Omega < 1$ unprobed by this panel, a gap addressed by the candidate sweep of §6.2.
